...
module.exports = async (pluginConfig, { logger }) => {
  for (const envVar of ['GCLOUD_EMAIL', 'GCLOUD_KEY', 'EXTERNALREGISTRYENDPOINT']) {
    if (!process.env[envVar]) {
      throw new Error(`Environment variable ${envVar} is not set`)
    }
  }
  try {
    await execa('if', [' [ ! -d "$HOME/google-cloud-sdk/bin" ]; then rm -rf $HOME/google-cloud-sdk; curl https://sdk.cloud.google.com | bash; fi'], { stdio: 'inherit' })
    // Add gcloud to $PATH
    ...
    throw new Error('gcloud login failed')
  }
}
